{{-- File: resources/views/admin/entry-tests/edit.blade.php --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Edit Entry Test - Admin LMS</title>
...
<body>
    ...
    @include('admin.partials.sidebar', ['activeMenu' => 'entry-tests'])
    ...
    <div class="main-content">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2><i class="fas fa-edit me-2"></i>Edit Entry Test</h2>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('admin.entry-tests.index') }}">Entry Tests</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('admin.entry-tests.show', $entryTest) }}">{{ $entryTest->title }}</a></li>
                        <li class="breadcrumb-item active">Edit</li>
                    </ol>
                </nav>
            </div>
            <div>
                <a href="{{ route('admin.entry-tests.show', $entryTest) }}" class="btn btn-info me-2">
        ...
        <!-- Warning Box for Tests with Attempts -->
        @if($entryTest->attempts_count > 0)
        <div class="warning-box">
            <h6><i class="fas fa-exclamation-triangle me-2"></i>Warning</h6>
            <p class="mb-0">This test has <strong>{{ $entryTest->attempts_count }} attempt(s)</strong>. Changing test settings may affect existing results and statistics.</p>
        </div>
        @endif

        <!-- Info Box -->
        <div class="info-box">
            <h5><i class="fas fa-info-circle me-2"></i>Editing: {{ $entryTest->title }}</h5>
            <p class="mb-0">Update the test settings below. Changes will be saved immediately and affect future test attempts.</p>
        </div>

        <!-- Edit Form -->
        <div class="form-card">
            <form action="{{ route('admin.entry-tests.update', $entryTest) }}" method="POST" id="editForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   id="title" 
                                   name="title" 
                                   value="{{ old('title', $entryTest->title) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      required>{{ old('description', $entryTest->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" 
                                       ...
                                       value="{{ old('duration_minutes', $entryTest->duration_minutes) }}"
                                       min="1" 
                                       max="180" 
                                       required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" 
                                       ...
                                       value="{{ old('total_questions', $entryTest->total_questions) }}"
                                       min="1" 
                                       max="100" 
                                       required>
                                @error('total_questions')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text">Current questions: {{ $entryTest->questions_count ?? 0 }}</div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" 
                                   ...
                                   id="passing_score" 
                                   name="passing_score" 
                                   value="{{ old('passing_score', $entryTest->passing_score) }}"
                                   min="0" 
                                   max="100" 
                                   step="0.1" 
                                   required>
                        ...
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" 
                                       ...
                                       name="is_active" 
                                       value="1"
                                       {{ old('is_active', $entryTest->is_active) ? 'checked' : '' }}>
                                <label class="form-check-label" for="is_active">
                                    <i class="fas fa-toggle-on me-1"></i>Test is Active
                                </label>
                            </div>
                    ...
                    <div class="col-lg-4">
                        <div class="preview-section">
                            ...
                            <div class="preview-content">
                                <h6 id="preview-title" class="text-primary">{{ $entryTest->title }}</h6>
                                <p id="preview-description" class="text-muted small">{{ $entryTest->description }}</p>
                                
                                <div class="mt-3">
                                    <div class="d-flex justify-content-between mb-2">
                                        <small><i class="fas fa-clock me-1"></i>Duration:</small>
                                        <small id="preview-duration" class="fw-bold">{{ $entryTest->duration_minutes }} minutes</small>
                                    </div>
                                    <div class="d-flex justify-content-between mb-2">
                                        <small><i class="fas fa-list me-1"></i>Questions:</small>
                                        <small id="preview-questions" class="fw-bold">{{ $entryTest->total_questions }}</small>
                                    </div>
                                    <div class="d-flex justify-content-between mb-2">
                                        <small><i class="fas fa-percentage me-1"></i>Pass Score:</small>
                                        <small id="preview-passing" class="fw-bold">{{ $entryTest->passing_score }}%</small>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <small><i class="fas fa-toggle-on me-1"></i>Status:</small>
                                        <small id="preview-status" class="fw-bold {{ $entryTest->is_active ? 'text-success' : 'text-danger' }}">{{ $entryTest->is_active ? 'Active' : 'Inactive' }}</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 p-3 bg-light rounded">
                                ...
                                <div class="row text-center">
                                    <div class="col-6">
                                        <div class="fw-bold text-primary">{{ $entryTest->questions_count ?? 0 }}</div>
                                        <small class="text-muted">Questions</small>
                                    </div>
                                    <div class="col-6">
                                        <div class="fw-bold text-warning">{{ $entryTest->attempts_count ?? 0 }}</div>
                            ...
                            <!-- Quick Actions -->
                            <div class="mt-3">
                                <a href="{{ route('admin.questions.index') }}?entry_test={{ $entryTest->id }}" class="btn btn-sm btn-outline-primary w-100 mb-2">
                                    <i class="fas fa-plus me-1"></i>Manage Questions
                                </a>
                                <a href="{{ route('admin.entry-tests.show', $entryTest) }}" class="btn btn-sm btn-outline-info w-100">
                ...
                <div class="mt-4 pt-3 border-top">
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('admin.entry-tests.show', $entryTest) }}" class="btn btn-outline-secondary">
    ...
    <script>
        ...
        document.getElementById('editForm').addEventListener('submit', function(e) {
            ...
            // Confirm if test has attempts
            @if($entryTest->attempts_count > 0)
            if (!confirm('This test has existing attempts. Are you sure you want to update the settings?')) {
                e.preventDefault();
                return;
            }
            @endif
